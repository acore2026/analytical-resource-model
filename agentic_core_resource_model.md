...
| Qwen3 service time for complex intent | 8 ms/request |
...
For intent requests that invoke Qwen3, the Qwen3 serving time is modeled as $8\ \mathrm{ms/request}$. Queueing delay is excluded from the primary model to keep the analysis easy to explain. When utilization becomes high, the model reports degraded, high-risk, or unstable capacity status instead of calculating a separate waiting-time term.

$$
D_{Q,\mathrm{service}} = 8\ \mathrm{ms/request}
$$

Where: $D_{Q,\mathrm{service}}$ is the Qwen3 inference service time per request; $8\ \mathrm{ms/request}$ is the analytical service-time assumption used for complex intent requests.
...
| Latency | Direct procedure latency, fixed agent latency, CPU-side intent work, and optional Qwen3 service time. | The primary latency estimate excludes queueing; high utilization is reported as capacity risk. |
...
The primary latency model intentionally excludes queueing delay. This keeps the paper model focused on per-request processing cost and resource capacity. If utilization is high, the result should be read as a capacity risk rather than a precise latency prediction.

$$
D_{\mathrm{nonintent},i} = D_{\mathrm{base},i} + D_{\mathrm{agent,nonintent}}
$$

Where: $D_{\mathrm{nonintent},i}$ is the no-queue latency of a non-intent request of type $i$; $D_{\mathrm{base},i}$ is the baseline deterministic procedure latency; $D_{\mathrm{agent,nonintent}}$ is the lightweight non-intent Agent latency.

For intent requests, the no-queue latency adds the fixed Agent latency, CPU-side intent work, and the configured fraction of requests that invoke Qwen3:

$$
D_{\mathrm{intent},i} = D_{\mathrm{base},i} + D_{\mathrm{agent,intent}} + C_{\mathrm{agent,intent}} + r_Q D_{Q,\mathrm{service}}
$$

Where: $D_{\mathrm{intent},i}$ is the no-queue latency of an intent request of type $i$; $D_{\mathrm{agent,intent}}$ is the fixed intent Agent latency; $C_{\mathrm{agent,intent}}$ is the CPU-side intent Agent work expressed as milliseconds on the request path; $r_Q$ is the Qwen3 invocation ratio; $D_{Q,\mathrm{service}}$ is the Qwen3 service time. Queueing delay is outside the primary model.
