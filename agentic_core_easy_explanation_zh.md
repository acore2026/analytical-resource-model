...
其中 $u$ 是原始利用率， $F(u)$ 是加入竞争开销后的有效利用率。这样负载越高，曲线越会向上弯曲。
...
- 当意图比例为 $20\%$ 时，NPU 利用率为 $62.1\%$。
- 当意图比例为 $30\%$ 时，NPU 利用率为 $96.9\%$，说明配置的 NPU 集群处于高风险状态但尚未过载。
- 当意图比例为 $40\%$ 时，NPU 利用率为 $134.1\%$，说明配置的 NPU 集群已经过载。
