...
模型使用基于 USL 的开销倍率，在系统繁忙时增加竞争和协调带来的额外需求：
...
其中 $u$ 是原始利用率， $F_{\mathrm{USL}}(u)$ 是加入竞争和协调开销后的有效利用率。这样负载越高，曲线越会向上弯曲。
...
- 当意图比例为 $20\%$ 时，NPU 利用率为 $60.7\%$ 。
- 当意图比例为 $30\%$ 时，NPU 利用率为 $95.8\%$ ，说明配置的 NPU 集群处于高风险状态但尚未过载。
- 当意图比例为 $40\%$ 时，NPU 利用率为 $136.0\%$ ，说明配置的 NPU 集群已经过载。
