...
This repository contains an analytical resource model for an agentic 6G core control-plane architecture. It is designed for paper preparation and capacity discussion, not as a deployment benchmark. The model estimates request volume, CPU cost, memory traffic, Qwen3/NPU sizing, network bandwidth, and latency under different user counts and intent ratios.
...
It then calculates request rate, intent request rate, Qwen3 token demand, effective nonlinear overhead by load band, required Qwen3 NPU count, CPU utilization, network utilization, and latency.
...
The web app lets you tune user count, per-user request frequencies, intent ratio, CPU capacity, RAM, network capacity, Qwen3 token settings, NPU sizing, and agent cost assumptions. Results update in the browser.
