...
分析结果表明：总用户/事件负载首先施压于确定性 CPU 处理，而提高意图流量会增加 CPU、内存流量、带宽和 Qwen3 token 需求。在生产 NPU 规划后，默认 `10%` Qwen3 调用场景在 `100%` 意图流量下需要 `524` 张 NPU，但满负载场景会因 CPU 超载而失稳，除非增加 CPU 容量、降低意图比例、降低 CPU 侧处理成本或引入准入控制。
