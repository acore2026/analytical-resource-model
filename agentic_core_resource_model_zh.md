...
| Qwen3 调用比例 | 意图请求的 10% |
| Qwen3 token 配置 | 2,000 input tokens + 100 output tokens = 2,100 tokens/request |
| Qwen3 token 能力 | 15,591.71 tokens/s/replica |
...
[GPUStack 的 Qwen3-30B-A3B on Ascend 910B 基准](https://docs.gpustack.ai/2.0/performance-lab/qwen3-30b-a3b/910b/) 报告，优化后的 Medium Prompt 场景在 `2,000 input tokens` 和 `100 output tokens` 配置下达到 `15,591.71 total tokens/s`。同一基准报告 `Mean TPOT = 194.54 ms`；该数值仅作为基准背景引用，不进入本文模型公式，因为本文估算的是资源容量。[vLLM-Ascend 文档](https://docs.vllm.ai/projects/ascend/en/v0.18.0/) 包含 Qwen3-30B-A3B 指引；对于 32 GB NPU 卡，模型采用 tensor parallel size $TP_Q=4$ 。
...
| 10% | 10.0% | 10,430 | 1,043 | 6,892,144 | 188.6 | 73.7% | 90.783 Gbps | 1,381.4% | 7.815 Gbps | 不稳定 |
| 20% | 20.0% | 20,860 | 2,086 | 40,072,285 | 210.4 | 82.2% | 128.164 Gbps | 8,031.6% | 8.827 Gbps | 不稳定 |
| 30% | 30.0% | 31,290 | 3,129 | 124,866,892 | 232.8 | 90.9% | 165.545 Gbps | 25,026.7% | 9.840 Gbps | 不稳定 |
| 40% | 40.0% | 41,720 | 4,172 | 286,602,430 | 255.9 | 100.0% | 202.926 Gbps | 57,442.9% | 10.855 Gbps | 不稳定 |
| 50% | 50.0% | 52,150 | 5,215 | 550,605,367 | 279.8 | 109.3% | 240.307 Gbps | 110,356.2% | 11.871 Gbps | 不稳定 |
| 60% | 60.0% | 62,580 | 6,258 | 942,202,168 | 304.6 | 119.0% | 277.688 Gbps | 188,842.8% | 12.890 Gbps | 不稳定 |
| 70% | 70.0% | 73,010 | 7,301 | 1,486,719,302 | 330.2 | 129.0% | 315.069 Gbps | 297,978.7% | 13.909 Gbps | 不稳定 |
| 80% | 80.0% | 83,440 | 8,344 | 2,209,483,234 | 356.7 | 139.4% | 352.451 Gbps | 442,840.1% | 14.931 Gbps | 不稳定 |
| 90% | 90.0% | 93,870 | 9,387 | 3,135,820,432 | 384.3 | 150.1% | 389.832 Gbps | 628,503.2% | 15.955 Gbps | 不稳定 |
| 100% | 100.0% | 104,300 | 10,430 | 4,291,057,362 | 412.9 | 161.3% | 427.213 Gbps | 860,043.8% | 16.980 Gbps | 不稳定 |
...
在配置 $128$ 张 NPU 用于 Qwen3 服务且 Qwen3 调用比例为 $10\%$ 的情况下，Medium Prompt token 配置是一个保守压力场景。NPU 利用率在 $10\%$ 意图比例时已经达到 $1,381.4\%$ ，在 $20\%$ 意图比例时达到 $8,031.6\%$ ，因此只要有显著比例的意图流量以该 prompt 大小调用 Qwen3，固定 NPU 集群就会过载。要在该负载下稳定运行，需要增加 NPU 容量、降低 Qwen3 调用比例、缩短 token 配置、提升服务吞吐、引入准入控制，或将大部分意图请求路由到轻量级非 LLM 处理路径。
...
6. [GPUStack Qwen3-30B-A3B on Ascend 910B benchmark](https://docs.gpustack.ai/2.0/performance-lab/qwen3-30b-a3b/910b/)。
   该基准提供本文使用的参考 token 能力：优化后的 Medium Prompt 场景下，Qwen3-30B-A3B 在 `2,000 input tokens` 和 `100 output tokens` 配置下达到 `15,591.71 total tokens/s`，`Mean TPOT = 194.54 ms`。TPOT 数值仅作为基准背景引用，不进入资源公式。
