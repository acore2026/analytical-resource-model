...
这种表述避免了不现实的假设：即每个意图请求都执行完整 Qwen3 生成。模型将总意图流量与需要 Qwen3 的子集分开计算。
...
该项是分析近似，不是精确电信系统仿真。平均、p95 和 p99 时延由确定性流程时延、固定 Agent 时延、CPU 排队、Qwen3 生产 NPU 排队和网络排队组成。生产规划结果会报告为了保持 Qwen3 token 利用率不超过目标值所需的 NPU 数量。
