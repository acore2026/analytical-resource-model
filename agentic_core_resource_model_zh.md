...
模型使用固定运行分档表示高负载竞争。先计算线性需求，再将对应的 CPU、网络或 Qwen3 服务负载输入连续负载分档函数 $F(u)$。每个更高负载分档的边际斜率都高于前一分档。
...
该模型是非线性的，因为有效成本按照运行分档变化，而不是始终以同一个斜率增长。该函数是连续的，因此在分档边界不会产生人为的利用率跳变。利用率图在负载分档切换处改变斜率是预期结果，不是插值错误。

令 $[x]_+=\max(x,0)$。连续负载分档函数为：
...
非线性负载分档函数用于表示高并发下有效服务效率下降，而不是表示单个请求的语义工作量发生变化。在正常运行状态下，模型等同于透明的线性基线；进入繁忙、高负载和临界状态后，每增加一单位负载都会额外消耗竞争、调度、内存搬移、排队和运行时协调等容量。
...
下表固定用户规模、事件频率和 Qwen3 集群规模，仅以固定 $10\%$ 步长改变全部请求中携带意图的比例。可见结果使用连续式负载分档非线性模型。意图比例扫描图使用 $1\%$ 采样展示细节，并标注 CPU 或 NPU 进入新负载分档的位置。

| 意图比例 | 总意图占比 | 意图 rps | Qwen3 rps | 有效 Qwen3 tokens/s | CPU 核 | CPU 利用率 | 内存流量 | NPU 利用率 | 网络带宽 | 平均时延 | p95 时延 | 状态 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 0% | 0.0% | 0 | 0 | 0 | 157.3 | 61.4% | 53.402 Gbps | 0.0% | 6.779 Gbps | 22.9 ms | 95.8 ms | 稳定 |
| 10% | 10.0% | 10,430 | 1,043 | 137,676 | 177.7 | 69.4% | 90.783 Gbps | 28.6% | 7.780 Gbps | 24.9 ms | 138.1 ms | 稳定 |
| 20% | 20.0% | 20,860 | 2,086 | 275,352 | 198.1 | 77.4% | 128.164 Gbps | 57.2% | 8.782 Gbps | 28.1 ms | 220.6 ms | 退化 |
| 30% | 30.0% | 31,290 | 3,129 | 437,268 | 219.5 | 85.7% | 165.545 Gbps | 90.9% | 9.783 Gbps | 34.9 ms | 348.9 ms | 高风险 |
| 40% | 40.0% | 41,720 | 4,172 | 652,518 | 243.5 | 95.1% | 202.926 Gbps | 135.6% | 10.784 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 50% | 50.0% | 52,150 | 5,215 | 872,800 | 271.2 | 105.9% | 240.307 Gbps | 181.3% | 11.786 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 60% | 60.0% | 62,580 | 6,258 | 1,093,082 | 299.5 | 117.0% | 277.688 Gbps | 227.1% | 12.787 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 70% | 70.0% | 73,010 | 7,301 | 1,313,363 | 327.9 | 128.1% | 315.069 Gbps | 272.9% | 13.788 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 80% | 80.0% | 83,440 | 8,344 | 1,533,645 | 356.3 | 139.2% | 352.451 Gbps | 318.7% | 14.789 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 90% | 90.0% | 93,870 | 9,387 | 1,753,926 | 384.6 | 150.2% | 389.832 Gbps | 364.4% | 15.791 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 100% | 100.0% | 104,300 | 10,430 | 1,974,208 | 413.0 | 161.3% | 427.213 Gbps | 410.2% | 16.792 Gbps | 不稳定 | 不稳定 | 不稳定 |
...
在配置 $128$ 张 NPU 用于 Qwen3 服务且 Qwen3 调用比例为 $10\%$ 的情况下，NPU 利用率在 $10\%$ 意图比例时为 $28.6\%$，在 $20\%$ 意图比例时为 $57.2\%$，在 $30\%$ 意图比例时为 $90.9\%$。当意图比例达到 $40\%$ 时，NPU 利用率超过 $100\%$，表示固定 NPU 集群已经过载。更高意图比例需要增加 NPU 容量、降低 Qwen3 调用比例、缩短 token 配置、提升服务吞吐或引入准入控制。
