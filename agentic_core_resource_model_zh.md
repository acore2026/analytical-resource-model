...
| Qwen3/NPU 服务 | 批处理效率下降、请求路由、副本调度、运行时协调、跨副本开销，以及 KV/cache 内存压力。 | 在计算副本和 NPU 数量前，将原始 token 需求转换为有效 token 需求。 |
...
下表固定用户规模和事件频率，仅以固定 $10\%$ 步长改变全部请求中携带意图的比例。可见结果使用分段式非线性负载模型。

| 意图比例 | 总意图占比 | 意图 rps | Qwen3 rps | 有效 Qwen3 tokens/s | CPU 核 | CPU 利用率 | 内存流量 | 规划后 Qwen3 利用率 | 所需生产 NPU | 网络带宽 | 平均时延 | p95 时延 | 状态 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 0% | 0.0% | 0 | 0 | 0 | 180.3 | 70.4% | 53.402 Gbps | 0.0% | 0 | 6.779 Gbps | 24.6 ms | 141.9 ms | 退化 |
| 10% | 10.0% | 10,430 | 1,043 | 158,327 | 200.7 | 78.4% | 90.783 Gbps | 65.8% | 64 | 7.780 Gbps | 28.1 ms | 231.8 ms | 退化 |
| 20% | 20.0% | 20,860 | 2,086 | 316,655 | 221.1 | 86.4% | 128.164 Gbps | 67.9% | 124 | 8.782 Gbps | 35.1 ms | 350.8 ms | 高风险 |
| 30% | 30.0% | 31,290 | 3,129 | 474,982 | 283.5 | 110.7% | 165.545 Gbps | 68.7% | 184 | 9.783 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 40% | 40.0% | 41,720 | 4,172 | 633,310 | 307.5 | 120.1% | 202.926 Gbps | 69.0% | 244 | 10.784 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 50% | 50.0% | 52,150 | 5,215 | 791,637 | 392.8 | 153.4% | 240.307 Gbps | 69.3% | 304 | 11.786 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 60% | 60.0% | 62,580 | 6,258 | 949,964 | 421.1 | 164.5% | 277.688 Gbps | 69.4% | 364 | 12.787 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 70% | 70.0% | 73,010 | 7,301 | 1,108,292 | 449.5 | 175.6% | 315.069 Gbps | 69.5% | 424 | 13.788 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 80% | 80.0% | 83,440 | 8,344 | 1,266,619 | 477.9 | 186.7% | 352.451 Gbps | 69.6% | 484 | 14.789 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 90% | 90.0% | 93,870 | 9,387 | 1,424,947 | 506.2 | 197.7% | 389.832 Gbps | 69.7% | 544 | 15.791 Gbps | 不稳定 | 不稳定 | 不稳定 |
| 100% | 100.0% | 104,300 | 10,430 | 1,583,274 | 534.6 | 208.8% | 427.213 Gbps | 69.7% | 604 | 16.792 Gbps | 不稳定 | 不稳定 | 不稳定 |
...
在分段式非线性负载模型和 $10\%$ Qwen3 调用比例下， $100\%$ 意图流量需要的 Qwen3 NPU 数量见上表。满负载场景会因 CPU 超载而失稳，除非增加 CPU 容量、降低意图比例、降低 CPU 侧处理成本或引入准入控制。
