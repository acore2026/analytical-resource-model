...
export const I18N: Record<Lang, Record<string, string>> = {
  en: {
    ...
    qwen3LoadMetric: "Qwen3/NPU util (%)",
    ...
    intentSweepDescription: "X-axis: intent-bearing share across all request types, sampled every 10 percentage points. Results use fixed nonlinear load bands. Y-axis: CPU, network, and Qwen3/NPU utilization.",
    xAxisLabel: "Intent ratio (%)",
    yAxisLabel: "CPU / Network / Qwen3 util (%)",
    intentSweepAria: "Line chart showing CPU utilization, network utilization, and Qwen3 NPU utilization by intent ratio.",
    userCountSweep: "User Count Sensitivity",
    userCountSweepDescription: "Intent ratio is fixed at 20%. Fixed nonlinear load bands add overhead as the system moves from normal to busy, high-load, and critical operation; Qwen3 line shows NPU utilization.",
    userCountXAxisLabel: "User count (million users)",
    userCountSweepAria: "Line chart showing CPU utilization, network utilization, and Qwen3 NPU utilization by user count at 20% intent ratio.",
    ...
    qwen3ClusterUtil: "Qwen3/NPU utilization is",
    stable: "stable",
    degraded: "degraded",
    high_risk: "high risk",
    unstable: "unstable",
  ...
  zh: {
    ...
    memoryMetric: "内存（GB）",
    qwen3LoadMetric: "Qwen3/NPU 利用率（%）",
    bandwidthMetric: "带宽（Gbps）",
    meanLatencyMetric: "平均时延（ms）",
    p95LatencyMetric: "p95 时延（ms）",
    p99LatencyMetric: "p99 时延（ms）",
    intentSweep: "意图比例扫描",
    intentSweepDescription: "横轴表示全部请求类型中携带意图的比例，每 10 个百分点采样一次。结果采用固定的非线性负载分档。纵轴表示 CPU、网络和 Qwen3/NPU 利用率。",
    xAxisLabel: "意图比例（%）",
    yAxisLabel: "CPU / 网络 / Qwen3 利用率（%）",
    intentSweepAria: "折线图，展示不同意图比例下的 CPU 利用率、网络利用率和 Qwen3 NPU 利用率。",
    userCountSweep: "用户规模敏感性",
    userCountSweepDescription: "意图比例固定为 20%。固定的非线性负载分档会在系统从正常、繁忙、高负载到临界状态时增加开销；Qwen3 折线表示 NPU 利用率。",
    userCountXAxisLabel: "用户数（百万）",
    userCountSweepAria: "折线图，展示 20% 意图比例下不同用户规模对应的 CPU 利用率、网络利用率和 Qwen3 NPU 利用率。",
    ...
    bottleneck: "瓶颈",
    intentTraffic: "意图流量为",
    qwen3ClusterUtil: "Qwen3/NPU 利用率为",
    stable: "稳定",
    degraded: "退化",
    high_risk: "高风险",
    unstable: "不稳定",
