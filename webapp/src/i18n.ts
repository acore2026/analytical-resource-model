...
export const I18N: Record<Lang, Record<string, string>> = {
  en: {
    ...
    intentSweepDescription: "X-axis: intent-bearing share across all request types, sampled every 1 percentage point. The convex nonlinear model adds contention overhead as load grows, so utilization curves bend upward instead of remaining straight.",
    ...
    userCountSweepDescription: "Intent ratio is fixed at 20%. User count is swept from 0.5M to 4.0M. The convex nonlinear model adds contention overhead as service volume grows; the NPU line shows inference-resource utilization.",
    ...
    tableMean: "Mean (ms)",
    tableStatus: "Status",
    curvatureNote: "Curvature = contention overhead as load grows",
    ...
    loadBandNote: "Nonlinear overhead uses a smooth convex curve: effective load = raw load + 0.15 x raw load squared.",
  ...
  zh: {
    ...
    intentSweep: "意图比例扫描",
    intentSweepDescription: "横轴表示全部请求类型中携带意图的比例，每 1 个百分点采样一次。凸性非线性模型会在负载升高时加入竞争开销，因此利用率曲线会向上弯曲，而不是保持直线。",
    xAxisLabel: "意图比例（%）",
    yAxisLabel: "CPU / 网络 / NPU 利用率（%）",
    intentSweepAria: "折线图，展示不同意图比例下的 CPU 利用率、网络利用率和 NPU 利用率。",
    userCountSweep: "用户规模敏感性",
    userCountSweepDescription: "意图比例固定为 20%。用户数从 0.5M 扫描到 4.0M。凸性非线性模型会在业务量升高时加入竞争开销；NPU 折线表示推理资源利用率。",
    ...
    tableMean: "平均值（ms）",
    tableStatus: "状态",
    curvatureNote: "曲线弯曲 = 负载升高带来的竞争开销",
    ...
    loadBandNote: "非线性开销采用平滑凸性曲线：有效负载 = 原始负载 + 0.15 x 原始负载平方。",
