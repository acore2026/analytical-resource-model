export const I18N = {
    en: {
        ...
        intentSweepDescription: "X-axis: intent-bearing share across all request types, sampled every 1 percentage point. This is a continuous load-band model; slope changes appear when CPU, network, or NPU enters a new load band.",
        ...
        userCountSweepDescription: "Intent ratio is fixed at 20%. User count is swept from 0.5M to 4.0M; continuous nonlinear load bands add overhead as the system moves from normal to busy, high-load, and critical operation. The NPU line shows inference-resource utilization.",
        ...
        tableStatus: "Status",
        bandTransition: "band",
        bandTransitionNote: "Slope changes = load-band transitions",
        ...
        loadBandNote: "Nonlinear overhead uses fixed load bands: normal below 60%, busy at 60-80%, high load at 80-90%, and critical above 90%.",
    ...
    zh: {
        ...
        intentSweep: "意图比例扫描",
        intentSweepDescription: "横轴表示全部请求类型中携带意图的比例，每 1 个百分点采样一次。这是连续式负载分档模型；当 CPU、网络或 NPU 进入新负载分档时会出现斜率变化。",
        xAxisLabel: "意图比例（%）",
        yAxisLabel: "CPU / 网络 / NPU 利用率（%）",
        intentSweepAria: "折线图，展示不同意图比例下的 CPU 利用率、网络利用率和 NPU 利用率。",
        userCountSweep: "用户规模敏感性",
        userCountSweepDescription: "意图比例固定为 20%。用户数从 0.5M 扫描到 4.0M；连续式非线性负载分档会在系统从正常、繁忙、高负载到临界状态时增加开销。NPU 折线表示推理资源利用率。",
        ...
        tableStatus: "状态",
        bandTransition: "分档",
        bandTransitionNote: "斜率变化 = 负载分档切换",
        ...
        loadBandNote: "非线性开销采用固定负载分档：60% 以下为正常，60-80% 为繁忙，80-90% 为高负载，90% 以上为临界。",
