export const I18N = {
    en: {
        ...
        intentSweepDescription: "X-axis: intent-bearing share across all request types, sampled every 10 percentage points. Results use fixed nonlinear load bands. Left Y-axis: CPU and network utilization. Right Y-axis: required Qwen3 NPUs.",
        ...
        userCountSweepDescription: "Intent ratio is fixed at 20%. Fixed nonlinear load bands add overhead as the system moves from normal to busy, high-load, and critical operation; Qwen3 line shows required NPU count.",
    ...
    zh: {
        ...
        intentSweep: "意图比例扫描",
        intentSweepDescription: "横轴表示全部请求类型中携带意图的比例，每 10 个百分点采样一次。结果采用固定的非线性负载分档。左纵轴表示 CPU 和网络利用率，右纵轴表示所需 Qwen3 NPU 数量。",
        xAxisLabel: "意图比例（%）",
        yAxisLabel: "CPU/网络利用率（%）· Qwen3 NPU 数",
        intentSweepAria: "折线图，展示不同意图比例下的 CPU 利用率、网络利用率和所需 Qwen3 NPU 数量。",
        userCountSweep: "用户规模敏感性",
        userCountSweepDescription: "意图比例固定为 20%。固定的非线性负载分档会在系统从正常、繁忙、高负载到临界状态时增加开销；Qwen3 折线表示所需 NPU 数量。",
